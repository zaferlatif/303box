(() => {
  ...
  // A tie or a slide belongs to the CURRENT step and connects it to the next one.
  ...
  function updateEffects(immediate = false) {
    if (!engine.ctx || !engine.fx) return;
    ...
    const set = (param, value) => {
      try { param.setTargetAtTime(value, now, tc); } catch (_) { param.value = value; }
    };

    // At 0% this is a true clean dry path. Distortion is parallel, never mandatory.
    set(engine.fx.dry.gain, 1 - dist * 0.28);
    set(engine.fx.drivePre.gain, 1 + dist * 8.5);
    set(engine.fx.driveWet.gain, dist * 0.58);

    // Tempo-synchronised dotted-eighth delay. The single DELAY knob controls send + feedback.
  ...
  function schedulePitchTransition(osc, current, next, boundary, dur) {
    ...
    if (current.expr.includes('S')) {
      // Pitch CV changes at the next step boundary while the gate remains open.
  ...
  function scheduleSegment(startStep, when) {
    ...
    filter.type = 'lowpass';
    // Less brutal than the previous engine: resonance shapes the filter, not a permanent distortion stage.
    ...
    for (let pos = 0; pos < segment.length; pos += 1) {
      ...
      if (pos < segment.length - 1) {
        ...
        schedulePitchTransition(osc, current, next, when + (pos + 1) * dur, dur);
      }
    }

    const end = when + segment.length * dur;
    const decay = knob('decay', 34);
    const singleDetached = segment.length === 1;
    const releaseAt = singleDetached ? when + dur * (first.gate === '○' ? 0.92 : 0.64) : end - Math.min(0.018, dur * 0.1);
    amp.gain.setTargetAtTime(0.0001, Math.max(when + 0.015, releaseAt), 0.035 + decay * 0.085);

    osc.connect(filter);
    filter.connect(amp);
    amp.connect(engine.input);
  ...
  function installIndependentNotePickers() {
    ...
    const persistBase = () => {
      localStorage.setItem('303box-note-base-octaves-v2', JSON.stringify($$('.note-input').map(input => Number(input.dataset.baseOctave || 0) ? 1 : 0)));
    };
  ...
  function init() {
    ...
    updateUi(false);
    window.__303boxPlaybackEngine = { start, stop, toggle, readStep, frequencyFor, updateEffects };
  }

  window.addEventListener('DOMContentLoaded', () => setTimeout(init, 100));
  window.addEventListener('load', () => setTimeout(() => { if (!window.__303boxPlaybackEngine) init(); }, 160));
})();