(() => {
  ...
  const COPY={
    en:{
      ...
      td3DirectTitle:'TD-3 DIRECT WRITE',td3DirectUsbOnly:'EXPERIMENTAL / USB ONLY',td3Group:'GROUP',td3Section:'SECTION',td3Pattern:'PATTERN',td3Verify:'VERIFY USB / SYSEX',td3BackupWrite:'BACKUP + WRITE',td3Restore:'RESTORE LAST BACKUP',td3ReadOnly:'READ TARGET ONLY',
      ...
      faqEyebrow:'FAQ',faqTitle:'Built to get out of the way.',faqLead:'No account, no project setup and no software installation.',footerProject:'303box is a Z3Z project.',footerPrivacy:'Privacy',footerDisclaimer:'Disclaimer'
    },
    tr:{
      ...
      td3DirectTitle:'TD-3 DOĞRUDAN YAZMA',td3DirectUsbOnly:'DENEYSEL / YALNIZ USB',td3Group:'GRUP',td3Section:'BÖLÜM',td3Pattern:'PATTERN',td3Verify:'USB / SYSEX DOĞRULA',td3BackupWrite:'YEDEKLE + YAZ',td3Restore:'SON YEDEĞİ GERİ YÜKLE',td3ReadOnly:'YALNIZ HEDEFİ OKU',
      ...
      faqEyebrow:'SSS',faqTitle:'Araya girmemek için tasarlandı.',faqLead:'Hesap yok, proje kurulumu yok, yazılım kurulumu yok.',footerProject:'303box bir Z3Z projesidir.',footerPrivacy:'Gizlilik',footerDisclaimer:'Sorumluluk'
    }
  };
  ...
  function translateHardwareGuide(){
    ...
    const td3Actions=['td3Verify','td3BackupWrite','td3Restore','td3ReadOnly'];$$('.td3-direct-actions button').forEach((el,index)=>{if(td3Actions[index])el.textContent=key(td3Actions[index])});
  }
  ...
  function translateLateModules(){
    ...
    $$('.footer-links a').forEach(link=>{if(link.matches('[data-disclaimer-link]'))link.textContent=key('footerDisclaimer');else if((link.getAttribute('href')||'').includes('privacy.html'))link.textContent=key('footerPrivacy')});
  }
