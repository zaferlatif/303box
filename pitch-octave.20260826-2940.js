(() => {
  ...
  let baseOctave=2,observer=null,normalizing=false,t8RecBusy=false;
  ...
  function normalizePickers(){
    $$('#patternSheet .note-input').forEach((input,i)=>{
      ...
      const picker=$(`[data-note-picker="${i}"],#patternSheet .note-picker-v2:nth-of-type(${i+1})`);
  ...
  function refreshTitles(){
    const cells=$$('#patternSheet .octave-cell');cells.forEach((cell,i)=>{const x=noteData(i),m=midiFor(x);cell.title=x.note?`${x.note}${x.oct} · MIDI ${m}`:`${isTR()?'Oktav':'Octave'} ${x.oct}`});
  }
  function normalizeCells(){
    ...
    normalizing=true;
    try{
      cells.forEach((cell,i)=>{const value=legacyToAbsolute(cell.textContent.trim().toUpperCase(),inputs[i]);cell.textContent=String(value);cell.dataset.value=String(value);cell.dataset.values=JSON.stringify(OCTAVES.map(String));cell.setAttribute('aria-label',`${isTR()?'Oktav':'Octave'} ${i+1}: ${value}`);inputs[i].dataset.baseOctave='0'});
  ...
  function shiftPattern(next){
    ...
    if(delta)$$('#patternSheet .octave-cell').forEach(cell=>{const n=Number(cell.textContent);cell.textContent=String(clamp((Number.isFinite(n)?n:baseOctave-delta)+delta,1,5))});
  ...
  function renderGlobal(){
    ...
    const row=$('#patternPitchControls');if(row){const title=row.querySelector('strong'),hint=row.querySelector('small');if(title)title.textContent=isTR()?'TEMEL OKTAV':'BASE OCTAVE';if(hint)hint.textContent=isTR()?'Her step OCTAVE satırından ayrıca ayarlanır.':'Each step can override it in the OCTAVE row.'}
    const label=$('#patternSheet [data-i18n="octave"]');if(label)label.textContent=isTR()?'OKTAV':'OCTAVE';
  }
  ...
  function patchOutput(output){
    ...
    try{output.send=function(data,timestamp){
      const msg=Array.from(data||[]),st=window.__303boxMidiRouter?.state,status=(msg[0]||0)&0xF0,ch=((msg[0]||0)&15)+1;
  ...
  function init(){
    ...
    document.addEventListener('click',e=>{if(e.target?.closest?.('#generateButton,#clearButton'))setTimeout(()=>{normalizeCells();refreshTitles()},0)},true);
    ...
    if(!observer){observer=new MutationObserver(()=>{if(!normalizing)queueMicrotask(settle)});observer.observe(document.documentElement,{childList:true,subtree:true})}
