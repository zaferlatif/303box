(() => {
  ...
  function injectStyle(){
    ...
    s.textContent=`
      ...
      .td3-direct-actions button[data-busy="true"]{opacity:1;box-shadow:inset 0 0 0 1px rgba(221,255,55,.22)}
  ...
  function familyCard(){
    const dialog=$('#hardwareGuideDialog');
    if(!dialog)return null;
    let card=$('.hardware-device-card[data-device~="td3"]',dialog)||$('.hardware-device-card[data-device~="td3mo"]',dialog);
    if(!card)return null;
    card.dataset.device='td3 td3mo';
    card.classList.add('primary');
    card.innerHTML=`<div class="hardware-device-title"><h3>${FAMILY}</h3><small>PRIMARY / BASS</small></div>
      <div class="hardware-capabilities">
        <div class="hardware-capability"><span>LIVE MIDI</span><strong class="good">Notes, accent velocity, slide overlap</strong></div>
        <div class="hardware-capability"><span>DIRECT SYSEX</span><strong class="warn">Read → backup → confirm → write → read-back verify</strong></div>
        <div class="hardware-capability"><span>TRANSPORT</span><strong>Live notes do not start stored patterns</strong></div>
        <div class="hardware-capability"><span>MODEL FAMILY</span><strong>${FAMILY}</strong></div>
      </div>`;
    return card;
  }
  ...
  function cleanCards(){
    ...
    $$('.hardware-device-card',dialog).forEach(card=>{
      ...
      if(ids.includes('td3')||ids.includes('td3mo'))return;
      card.remove();
    });
  }
  ...
  function targetLabel(){
    ...
    const number=Math.max(1,Math.min(8,Number($('#td3WriteNumber')?.value||1)));
    return `${['I','II','III','IV'][Math.max(0,Math.min(3,group))]} / ${section}${number}`;
  }
    ...
    if(!box){
      ...
      box.innerHTML=`<div class="td3-direct-head"><strong>${FAMILY} DIRECT WRITE</strong><small>EXPERIMENTAL / USB ONLY</small></div>
        <div class="td3-direct-target">
          ...
          <label><span>PATTERN</span><select id="td3WriteNumber">${Array.from({length:8},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join('')}</select></label>
        </div>
      ...
      $$('select',box).forEach(select=>select.addEventListener('change',()=>{
        const status=$('#td3DirectStatus');
        if(status&&!window.__303boxHardwareFidelity?.state?.busy){
          status.className='td3-direct-status warn';
          status.textContent=`${targetLabel()} — ${say('target selected. Nothing has been written.','hedef seçildi. Henüz hiçbir şey yazılmadı.')}`;
        }
      }));
    }
    const head=$('.td3-direct-head strong',box);
    if(head)head.textContent=`${FAMILY} DIRECT WRITE`;
    const status=$('#td3DirectStatus',box);
    if(status&&!status.textContent.trim())status.textContent=`${targetLabel()} — ${say('target selected. Nothing has been written.','hedef seçildi. Henüz hiçbir şey yazılmadı.')}`;
    return true;
  }
  ...
  function init(){
    ...
    window.__303boxHardwareGuide={version:'2960',open:openGuide,refresh};
  }
