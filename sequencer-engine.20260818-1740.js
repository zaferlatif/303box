(() => {
  ...
  function loadStyle(path){return new Promise(resolve=>{const clean=path.split('?')[0];const existing=[...document.querySelectorAll('link[rel="stylesheet"]')].find(x=>(x.getAttribute('href')||'').split('?')[0]===clean&&x.dataset.runtimeFinal==='2000');if(existing){resolve();return}const link=document.createElement('link');link.rel='stylesheet';link.href=withVersion(path);link.dataset.runtimeFinal='2000';link.addEventListener('load',()=>resolve(),{once:true});link.addEventListener('error',()=>{console.warn('[303box] stylesheet failed',path);resolve()},{once:true});document.head.appendChild(link)})}
  function loadScript(path){return new Promise(resolve=>{const clean=path.split('?')[0];const found=[...document.scripts].find(x=>(x.getAttribute('src')||'').split('?')[0]===clean&&x.dataset.runtimeFinal==='2000');if(found){resolve();return}const s=document.createElement('script');s.src=withVersion(path);s.async=false;s.dataset.runtimeFinal='2000';s.addEventListener('load',()=>resolve(),{once:true});s.addEventListener('error',()=>{console.warn('[303box] module failed',path);resolve()},{once:true});document.head.appendChild(s)})}
  ...
  (async()=>{
    try{
      ...
      await domReady();await wait(180);await waitForFinalControls();await windowLoaded();await wait(220);applyFinalState();await waitFonts();await waitMinimumBoot();clearTimeout(watchdog);await reveal('complete');
